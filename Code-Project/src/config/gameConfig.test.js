...
test("validateGameConfig requires all four game timings", () => {
  const configWithoutMinimumLoading = structuredClone(gameConfig.GAME_CONFIG);
  delete configWithoutMinimumLoading.timings.minimumLoadingMs;

  assert.throws(
    () => gameConfig.validateGameConfig(configWithoutMinimumLoading),
